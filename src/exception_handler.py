...
class ExceptionHandler(commands.Cog):
    ...
    async def on_command_error(self, ctx: commands.Context, error):
        ...
        if isinstance(err, commands.MissingRequiredArgument):
            ...
            missing_roles = '{}, or {}'.format(", ".join(err.missing_roles[:-1]), err.missing_roles[-1])
            emb = nextcord.Embed(
    ...
    def gen_dennis_embed(self, exception) -> nextcord.Embed:
        exc = "".join(traceback.format_exception(etype=type(exception), value=exception, tb=exception.__traceback__))
